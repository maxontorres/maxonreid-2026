...
export default function SkillsSection() {
  ...
  return (
    <section className="py-24 px-0 bg-gradient-to-b from-transparent to-[#080808]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {skillCategories.map((category, index) => (
            <div key={index} className="bg-white/[0.02] border border-white/[0.06] rounded-xl p-6 hover:bg-white/[0.03] transition-colors">
              <h3 className="text-xl font-semibold mb-4 text-[#D4A843]">{category.title}</h3>
              <ul className="space-y-2">
                {category.items.map((skill, skillIndex) => (
                  <li key={skillIndex} className="text-[#AEACA6] flex items-center gap-2">
                    <span className="w-1.5 h-1.5 bg-[#D4A843] rounded-full"></span>
                    {skill}
                  </li>
                ))}
              </ul>
